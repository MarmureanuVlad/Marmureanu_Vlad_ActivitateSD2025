...
int calculeazaHash(const char* nume, int dimensiune) { // puteam sa avem si alta cheie dupa care sa calc hash, nu neaparat nume
...
int main() {
	HashTable ht = citireMasiniDinFisier("masini2.txt", 5);

	//afisareTabelaDeMasini(ht);

	Masina m = getMasinaDupaCheie(ht, "Gigi");
	if (m.id != -1) {
		afisareMasina(m);
		free(m.model);
		free(m.numeSofer);
